...
public class TemplateHashtableTests extends MemoryTrackingAssertNoLeaks {
  ...
  public void testCreate() {
    try (Cleaner cleaner = new Cleaner()) {
      // GIVEN
      long hashTable = TemplateHashtable.allocate(cleaner, 10, .75f);
      // WHEN
      hashTable = fill(hashTable, cleaner);
  ...
  private void doTestGet(Cleaner cleaner, int initialCapacity) {
    // GIVEN
    long hashTable = TemplateHashtable.allocate(cleaner, initialCapacity, .75f);
    // WHEN
    hashTable = fill(hashTable, cleaner);
    System.out.println(TemplateHashtable.toString(hashTable));
    // THEN
  ...
  private long fill(long hashTable, Cleaner cleaner) {
...
//    System.out.println(TemplateHashtable.toString(hashTable));
    long keys = TemplateHashtable.keys(hashTable, cleaner);
