...
public class LongHashtable {
  ...
  private static class Node {
    ...
    static long getKey(MemorySegment address) {
      return address.get(ValueLayout.JAVA_LONG_UNALIGNED, keyOffset);
    }

    static long getValue(MemorySegment address) {
      return address.get(ValueLayout.JAVA_LONG_UNALIGNED, valueOffset);
    }
    ...
    static void setKey(MemorySegment address, long key) {
      address.set(ValueLayout.JAVA_LONG_UNALIGNED, keyOffset, key);
    }

    static void setValue(MemorySegment address, long value) {
      address.set(ValueLayout.JAVA_LONG_UNALIGNED, valueOffset, value);
    }
  ...
  public static MemorySegment put(MemorySegment address, long key, long value) {
    ...
    if (bucketNode == 0) { // empty bucket
      ...
      for (long node = bucketNode; node != 0; node = Node.getNext(node)) {
        ...
        long nodeKey = Node.getKey(nodeMs);
        if (nodeKey == key) {
          // replace node
  ...
  private static MemorySegment resizeIfNeeded(MemorySegment address, int capacity) {
    ...
    MemorySegment newAddress = LongHashtable.allocate(null, newCapacity, loadFactor);
    ...
    for (int bucketIdx = 0; bucketIdx < capacity; bucketIdx++) {
      ...
      for (long node = bucketNode; 0 != node; node = Node.getNext(node)) {
        MemorySegment nodeMs = Node.of(node);
        LongHashtable.put(newAddress, Node.getKey(nodeMs), Node.getValue(nodeMs));
      }
    }
  ...
  public static long get(MemorySegment address, long key) {
    int hashCode = Long.hashCode(key);
  ...
  public static boolean containsKey(MemorySegment address, long key) {
    int hashCode = Long.hashCode(key);
   ...
   */
  public static long remove(MemorySegment address, long key) {
    int hashCode = Long.hashCode(key);
    ...
    for (long prevNode = 0, node = bucketNode; node != 0; node = Node.getNext(node)) {
      ...
      long nodeKey = Node.getKey(nodeP);
      if (nodeKey == key) {
        // remove node
        long value = Node.getValue(nodeP);
        if (prevNode != 0) {
  ...
  public static MemorySegment keys(MemorySegment address, Arena arena) {
    MemorySegment keysArrayAddr = LongArray.allocate(arena, getSize(address));
    int capacity = getCapacity(address);
    int i = 0;
    for (int bucketIdx = 0; bucketIdx < capacity; bucketIdx++) {
      long bucketOffset = bucketsOffset + bucketIdx * LONG_SIZE;
      ...
      for (long node = bucketNode; 0 != node; node = Node.getNext(node)) {
        LongArray.set(keysArrayAddr, i++, Node.getKey(Node.of(node)));
      }
    }
