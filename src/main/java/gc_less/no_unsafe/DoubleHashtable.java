...
public class DoubleHashtable {
  ...
  private static class Node {
    ...
    static double getKey(MemorySegment address) {
      return address.get(ValueLayout.JAVA_DOUBLE_UNALIGNED, keyOffset);
    }

    static double getValue(MemorySegment address) {
      return address.get(ValueLayout.JAVA_DOUBLE_UNALIGNED, valueOffset);
    }
    ...
    static void setKey(MemorySegment address, double key) {
      address.set(ValueLayout.JAVA_DOUBLE_UNALIGNED, keyOffset, key);
    }

    static void setValue(MemorySegment address, double value) {
      address.set(ValueLayout.JAVA_DOUBLE_UNALIGNED, valueOffset, value);
    }
  ...
  public static MemorySegment put(MemorySegment address, double key, double value) {
    ...
    if (bucketNode == 0) { // empty bucket
      ...
      for (long node = bucketNode; node != 0; node = Node.getNext(node)) {
        ...
        double nodeKey = Node.getKey(nodeMs);
        if (nodeKey == key) {
          // replace node
  ...
  private static MemorySegment resizeIfNeeded(MemorySegment address, int capacity) {
    ...
    MemorySegment newAddress = DoubleHashtable.allocate(null, newCapacity, loadFactor);
    ...
    for (int bucketIdx = 0; bucketIdx < capacity; bucketIdx++) {
      ...
      for (long node = bucketNode; 0 != node; node = Node.getNext(node)) {
        MemorySegment nodeMs = Node.of(node);
        DoubleHashtable.put(newAddress, Node.getKey(nodeMs), Node.getValue(nodeMs));
      }
    }
  ...
  public static double get(MemorySegment address, double key) {
    int hashCode = Double.hashCode(key);
  ...
  public static boolean containsKey(MemorySegment address, double key) {
    int hashCode = Double.hashCode(key);
   ...
   */
  public static double remove(MemorySegment address, double key) {
    int hashCode = Double.hashCode(key);
    ...
    for (long prevNode = 0, node = bucketNode; node != 0; node = Node.getNext(node)) {
      ...
      double nodeKey = Node.getKey(nodeP);
      if (nodeKey == key) {
        // remove node
        double value = Node.getValue(nodeP);
        if (prevNode != 0) {
  ...
  public static MemorySegment keys(MemorySegment address, Arena arena) {
    MemorySegment keysArrayAddr = DoubleArray.allocate(arena, getSize(address));
    int capacity = getCapacity(address);
    int i = 0;
    for (int bucketIdx = 0; bucketIdx < capacity; bucketIdx++) {
      long bucketOffset = bucketsOffset + bucketIdx * LONG_SIZE;
      ...
      for (long node = bucketNode; 0 != node; node = Node.getNext(node)) {
        DoubleArray.set(keysArrayAddr, i++, Node.getKey(Node.of(node)));
      }
    }
