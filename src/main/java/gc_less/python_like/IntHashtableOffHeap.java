...
public class IntHashtableOffHeap {
  ...
  private int insert(int key, int value) {
    ...
    for (int storageLen = (int) storage.byteSize(),
            delta = 2 * intSize,
            bucketIdxOffset = bucket * delta,
            i = 0; /*i<capacity*/
        ;
        i++, bucketIdxOffset += delta) {
  ...
  private void resizeIfNeeded() {
    if (size > sizeMaxLoad) {
      ...
      for (int storageLen = (int) oldStorage.byteSize(), delta = 2 * intSize, bucketIdxOffset = 0;
          bucketIdxOffset < storageLen;
          bucketIdxOffset += delta) {
